...
const register = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Greetings newest member! Welcome to the System!' });
    } catch (err) {
...
const login = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: user.id, username: user.username }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_ACCESS_EXPIRATION_TIME });
