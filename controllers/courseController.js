...
const getCourseById = async (req, res) => {
    ...
    try {
        ...
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Course non-existent' });
        }
...
const deleteCourse = async (req, res) => {
    ...
    try {
        ...
        res.json({ message: 'Thanos snapped his fingers'});
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};
...
module.exports = { createCourse, getCourseById, getAllCourses, updateCourse, deleteCourse };