...
const getDeptById = async (req, res) => {
    ...
    try {
        ...
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Department non-existent.' });
        }
...
const deleteDept = async (req, res) => {
    ...
    try {
        ...
        res.json({ message: 'Department destroyed'});
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};
...
module.exports = { createDept, getDeptById, getAllDept, updateDept, deleteDept };